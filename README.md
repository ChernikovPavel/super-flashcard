...
- Авторизаия (Модель user, ручки, фронт)                                                // Султан
- Личный кабинет (статистика) (Моделей нет, ручки, фрон + опционально красивые графики) // Паша 
- Игра (Модели, ручки, фронт)                                                           // Алексей

2. Фичи:

- Presist (использование Local Storage для хранения состояний)                          // Алексей
- Модальное окно с инпутом на ввод ответа, ограниченное по времени при клике на вопрос (Реализует тот, кто занимается игрой)
...
- Расширенная статистика (рейтинг) победителей (личный кабинет)                         // Паша
